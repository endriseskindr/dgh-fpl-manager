...
## Changeable EAS project ID

The app configuration does not hard-code an EAS project ID. Set `EAS_PROJECT_ID` in the environment used for the build, for example: `EAS_PROJECT_ID=<your-project-id> eas build --platform android --profile release-apk`. For EAS-managed environments, store the variable in the appropriate EAS environment and select that environment for the build.
